...
@interface testViewController ()<UIWebViewDelegate, UIScrollViewDelegate, UIGestureRecognizerDelegate,UIPickerViewDataSource,UIPickerViewDelegate,NJKWebViewProgressDelegate,UIAlertViewDelegate>
{
    UIWebView *mWebView;
    UIWebView *mLastView;
    UIWebView *mNextView;
...
-(void)navClick:(UIButton *)sender
{
    ...
    if (sender.tag == 401) {
        onlyLouzhuViewController *vc =[[onlyLouzhuViewController alloc]init];
        vc.bID = self.bID;
        vc.fid = self.fid;
        vc.title = self.title;
...
//    webView.opaque = NO;
    ...
    [webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://www.iwatch365.com/json/barnr.php?fid=%@&p=%d&uid=0",self.fid,page]]]];
...
-(void)confirmClick
{
    ...
    [mNextView stopLoading];
    [mWebView stopLoading];
    [mLastView stopLoading];
    mNextView = [self GetWebView:nowPage+1];
    mWebView = [self GetWebView:nowPage];
    mLastView = [self GetWebView:nowPage-1];
    mLastView.center = CGPointMake(self.view.frame.size.width/2, -self.view.frame.size.height/2);
