...
+ (BOOL)IsValidNickName:(NSString *)string {
    ...
    for (int i = 0; i < string.length; i ++) {
        bFind = NO;
        unsigned short hanzi = [string characterAtIndex:i];
        int index = hanzi - HANZI_START;
        if (index >= 0 && index <= HANZI_COUNT) {
            bFind = YES;
        }
        else if (hanzi >= '0' && hanzi <= '9') {
            bFind = YES;
        }
        else if (hanzi >= 'a' && hanzi <= 'z') {
            bFind = YES;
...
+ (NSString *)GetChinesePinyin:(NSString *)string {
    ...
    for (int i = 0; i < iLen; i++) {
        ...
        if (strRes.length<6) {
            NSArray *resarray = [NSArray arrayWithObjects:@"      ", @"     ", @"    ", @"   ", @"  ", @" ", nil];
            if (strRes.length == 0) {
                
            }
            strRes = [strRes stringByAppendingString:[resarray objectAtIndex:strRes.length]];
        }
...
+ (NSComparisonResult)CompareString:(NSString *)str1 :(NSString *)str2 {
    NSString *dstStr1 = [ChinesePinyin GetChinesePinyin:str1];
    NSString *dstStr2 = [ChinesePinyin GetChinesePinyin:str2];
    return [dstStr1 compare:dstStr2];
}
