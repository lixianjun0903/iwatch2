//
//  RecommandDetailViewController.m
//  IWatchII
//
...
#import "UMSocialQQHandler.h"
@interface RecommandDetailViewController ()<UIWebViewDelegate,UIAlertViewDelegate,UIGestureRecognizerDelegate>
{
    UIWebView *mWebView;
...
@implementation RecommandDetailViewController
...
- (void)viewDidLoad
{
    ...
    mWebView = [[UIWebView alloc] initWithFrame:CGRectMake(2, 0, self.view.frame.size.width-2, self.view.frame.size.height)];
    mWebView.opaque = NO;
//    mWebView.scalesPageToFit = YES;
    mWebView.delegate = self;
    mWebView.backgroundColor = self.view.backgroundColor;
    mWebView.scrollView.contentInset = UIEdgeInsetsMake(110, 0, 50, 0);
    ...
    [leftBtn addTarget:self action:@selector(LoadNextPage) forControlEvents:UIControlEventTouchUpInside];
    ...
    [rightBtn addTarget:self action:@selector(LoadLastPage) forControlEvents:UIControlEventTouchUpInside];
    [botView addSubview:rightBtn];
    
    [self LoadRecommandDetail];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    mWebView.scrollView.contentOffset = CGPointMake(0, -110);
}
...
- (void)LoadRecommandDetail {
    if (_mDownManager) {
        return;
    }
    [self StartLoading];
    NSString *urlstr = [NSString stringWithFormat:@"%@json_watch.php?t=24&fid=%@", SERVER_URL, self.mInfo.fid];
    textUrl = urlstr;
...
- (void)OnLoadFinish:(ImageDownManager *)sender {
    ...
    if (dict && [dict isKindOfClass:[NSDictionary class]]) {
        ...
        if (array && [array isKindOfClass:[NSArray class]] && array.count>0) {
            ...
            finalPicArray =[NSMutableArray arrayWithCapacity:0];
            NSString *pics = tmpDict[@"pics"];
            if (![pics isEqualToString:@""]) {
                NSArray *picArray = [pics componentsSeparatedByString:@"http:"];
                for ( int i =1; i < picArray.count; i++) {
                    NSString *tempStr = picArray[i];
                    NSArray *tempArray = [tempStr componentsSeparatedByString:@".jpg"];
                    NSString *str = [NSString stringWithFormat:@"http:%@.jpg",tempArray[0]];
                    [finalPicArray addObject:str];
                }
                if (finalPicArray.count==0) {
                    mWebView.scrollView.contentInset = UIEdgeInsetsMake(0, 0, 50, 0);
                    mWebView.autoresizingMask = UIViewAutoresizingFlexibleHeight;
                }
                else
                {
...
//            NSString *datetime = [UserInfoManager GetFormatDateByInterval:[self.mInfo.updatetime intValue]];
            content = [NSString stringWithFormat:@"<html><head><style type=\"text/css\">img {width: 100%%}</style></head><body><div style=\"font-size:20px\"><span style=\"font-weight:bold\">%@</span><div style=\"font-size:16px;color:#888888;text-align:right\">%@</div><img src=\"file://%@\" /><br><div style=\"text-align:justify\"><span style=\"line-height:25px;font-size:13px\">%@</span></div></body></html>", self.mInfo.title,str1,lineimage, content];
...
- (void)LoadNextPage {
    ...
    NSString *urlstr = [NSString stringWithFormat:@"%@json_watch.php?t=21&fid=%@", SERVER_URL, self.mInfo.fid];
    self.mDownManager = [[ImageDownManager alloc] init];
    _mDownManager.delegate = self;
...
- (void)LoadLastPage {
    ...
    [self StartLoading];
    //www.iwatch365.com/json/iphone/json_watch.php?t=28&uid=1&fid=1
    NSString *urlstr = [NSString stringWithFormat:@"%@json_watch.php?t=22&fid=%@", SERVER_URL, self.mInfo.fid];
    self.mDownManager = [[ImageDownManager alloc] init];
    _mDownManager.delegate = self;
...
-(void)createSC1
{
    UIScrollView *scrollView1 = [[UIScrollView alloc] initWithFrame:CGRectMake(0, -110, self.view.frame.size.width, 110)];
    scrollView1.backgroundColor = [UIColor colorWithWhite:0.92 alpha:1.0];
    scrollView1.showsHorizontalScrollIndicator = NO;
    [mWebView.scrollView addSubview:scrollView1];
    int iLeft = 7;
    
    for (int i = 0; i < finalPicArray.count; i ++) {
        NetImageView *touchView = [[NetImageView alloc] initWithFrame:CGRectMake(iLeft, 7, 96, 96)];
        [touchView GetImageByStr:finalPicArray[i]];
        touchView.mDefaultImage = [UIImage imageNamed:@"65@2x.png"];
        touchView.tag = 100 + i;
        [touchView addGestureRecognizer:[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tap:) ]];
        [scrollView1 addSubview:touchView];
        iLeft += 103;
    }
    scrollView1.contentSize = CGSizeMake(iLeft, scrollView1.frame.size.height);
...
-(void)createSC2
{
    ...
    for (int i = 0; i < 8; i ++) {
        ...
        TouchView *touchView = [[TouchView alloc] initWithFrame:CGRectMake(iLeft, 7, 96, 96)];
        touchView.image = [UIImage imageNamed:imagename];
        touchView.delegate = self;
        touchView.OnViewClick = @selector(OnImageClick:);
        [scrollView1 addSubview:touchView];
...
- (void)OnImageClick:(TouchView *)sender {
    ...
    NSArray *picArray = @[@"watchicon001.png",@"watchicon002.png",@"watchicon003.png",@"watchicon004.png",@"watchicon001.png",@"watchicon002.png",@"watchicon003.png",@"watchicon004.png"];
    ctrl.picArray = picArray;
    ctrl.page = 0;
    [ctrl loadPics:0];
...
{
    [self tapShadow];
    NSString *shareText =self.mInfo.title;             //分享内嵌文字
    //分享内嵌图片
